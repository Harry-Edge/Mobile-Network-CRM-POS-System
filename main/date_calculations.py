...
class DateTimeCalculations:
    ...
    def calculate_annual_upgrade_fee(self):

        if self.ctn.annual_upgrade is True:
            if self.today < self.ctn.contract_start + timedelta(182):
                return 200
            elif self.today >= self.ctn.contract_start + timedelta(365):
                return 0
            elif self.today >= self.ctn.contract_start + timedelta(182):
                return 100
